...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            if history:
                ...
                await self.send(
                    text_data=json.dumps({"mensagens": messages}, cls=DjangoJSONEncoder)
                )
                return
            ...
            await self.channel_layer.group_send(
                self.group_name, {"type": "chat_message", "mensagens": messages}
            )
    ...
    async def chat_message(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"mensagens": event["mensagens"]}, cls=DjangoJSONEncoder
            )
        )
    ...
    def save_message(self, autor_id, texto, chamado_id):
        try:
            ...
            Mensagem.objects.create(autor=autor, texto=texto, chamado_id=chamado_id)
        except Exception as e:
    ...
    def get_messages_by_chamado(self, chamado_id):
        try:
            messages = Mensagem.objects.filter(chamado_id=chamado_id).order_by(
                "data_envio"
            )
            return [
                {
                    "id": msg.pk,
                    "autor": {"id": msg.autor.pk, "username": msg.autor.username},
                    "texto": msg.texto,
