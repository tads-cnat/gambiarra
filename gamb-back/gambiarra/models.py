...
class Chamado(models.Model):
    ...
    code = models.UUIDField(default=uuid.uuid4, editable=False, unique=True) # Added editable=False and unique=True
    ...
    acessorio = models.CharField(max_length=255, blank=True, null=True) # Added acessorio field
    ...
    def clean(self):
        ...
        if len(self.titulo) > 50:
            raise ValidationError({'titulo': "O título pode conter no máximo 50 caracteres"})

        if not self.descricao:
            raise ValidationError({'descricao': "A descrição é obrigatória"})
        if len(self.descricao) > 240: # Note: TextField max_length is usually for forms, not DB.
            ...
            raise ValidationError({'descricao': "A descrição pode conter no máximo 255 caracteres"})

        if not self.item:
            raise ValidationError({'item': "O modelo é obrigatório"})
        
        if self.acessorio and len(self.acessorio) > 255:
            raise ValidationError({'acessorio': "O acessório pode conter no máximo 255 caracteres"})
