...
class UserRegistrationSerializer(serializers.ModelSerializer):
    email = serializers.EmailField(
        required=True,
        validators=[UniqueValidator(queryset=User.objects.all())] #checks if the email is unique
    )
    ...
    class Meta:
        model = User
        fields = ('username', 'email', 'password1', 'password2') #fields to put in the register process
